...
CameraPublisher::CameraPublisher()
    : Node("camera_publisher"), m_logger(this->get_logger())
{
    initLaunchParams();
                                      ...
                                      const AS_SDK_CAM_MODEL_E &cam_type)
{
    ...
    case AS_CAMERA_ATTR_LNX_USB:
        if ((m_launch_param.usb_bus_num != -1) && (strcmp(m_launch_param.usb_port_nums, "null") != 0))
        {
            ...
            {
                param.open = false;
                RCLCPP_WARN_STREAM(m_logger, "found a dev but cannot match the specified camera, launch bus num " << m_launch_param.usb_bus_num << ", port num " << m_launch_param.usb_port_nums << ", but found bus num " << attr_t.attr.usbAttrs.bnum << ", port num " << attr_t.attr.usbAttrs.port_numbers);
                return -1;
            }
            else
            {
    ...
    case AS_CAMERA_ATTR_LNX_USB:
        ...
        {
            if ((it->attr_s.attr.usbAttrs.bnum == attr_t.attr.usbAttrs.bnum) && (strcmp(it->attr_s.attr.usbAttrs.port_numbers, attr_t.attr.usbAttrs.port_numbers) == 0))
            {
    ...
    case AS_CAMERA_ATTR_NET:
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++)
        {
            if ((it->attr_s.attr.netAttrs.port == attr_t.attr.netAttrs.port) && (strcmp(it->attr_s.attr.netAttrs.ip_addr, attr_t.attr.netAttrs.ip_addr) == 0))
            {
                RCLCPP_INFO_STREAM(m_logger, "reconnected, update the camera info");
                param.start = true;
                it->pCamera = pCamera;
    ...
    if (!reconnected)
    {
        ...
        case AS_SDK_CAM_MODEL_NUWA_HP60V:
            stPublisherInfo.depth_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/depth" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.depth_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth" + std::to_string(dev_idx) + "/image_raw", 100);
            stPublisherInfo.ir_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/ir" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.ir_img_pub = this->create_publisher<sensor_msgs::msg::Image>("~/ir" + std::to_string(dev_idx) + "/image", 100);
            stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" + std::to_string(dev_idx) + "/points", 100);
            break;
        case AS_SDK_CAM_MODEL_CHANGJIANG_B:
            stPublisherInfo.depth_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/depth" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.depth_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth" + std::to_string(dev_idx) + "/connectDepth",
                                                                                            100);
            stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" + std::to_string(dev_idx) + "/points", 100);
            break;
        ...
        case AS_SDK_CAM_MODEL_TANGGULA_B:
            stPublisherInfo.depth_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/depth" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.depth_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth" + std::to_string(dev_idx) + "/image_raw", 100);
            stPublisherInfo.rgb_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/rgb" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.rgb_img_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/rgb" + std::to_string(dev_idx) + "/image", 100);
            stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" + std::to_string(dev_idx) + "/points", 100);
            break;
        ...
        case AS_SDK_CAM_MODEL_KUNLUN_A:
            stPublisherInfo.depth_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/depth" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.depth_odd_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth" + std::to_string(dev_idx) + "/odd_image_raw", 100);
            stPublisherInfo.depth_even_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth" + std::to_string(dev_idx) + "/even_image_raw", 100);
            stPublisherInfo.depth_merge_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth" + std::to_string(dev_idx) + "/merge_image_raw", 100);
            stPublisherInfo.peak_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/peak" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.peak_odd_img_pub = this->create_publisher<sensor_msgs::msg::Image>("~/peak" + std::to_string(dev_idx) + "/odd_image", 100);
            stPublisherInfo.peak_even_img_pub = this->create_publisher<sensor_msgs::msg::Image>("~/peak" + std::to_string(dev_idx) + "/even_image", 100);
            stPublisherInfo.depth_points_odd_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" + std::to_string(dev_idx) + "/odd_points", 100);
            stPublisherInfo.depth_points_even_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" +
                                                                                                              std::to_string(dev_idx) + "/even_points",
                                                                                                          100);
            stPublisherInfo.depth_points_merge_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" +
                                                                                                               std::to_string(dev_idx) + "/merge_points",
                                                                                                           100);
            break;
        case AS_SDK_CAM_MODEL_KUNLUN_C:
            ...
            stPublisherInfo.rgb_img_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/rgb" + std::to_string(dev_idx) + "/image", 100);
            stPublisherInfo.peak_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/peak" + std::to_string(dev_idx) + "/camera_info", 5);
            stPublisherInfo.peak_img_pub = this->create_publisher<sensor_msgs::msg::Image>("~/peak" + std::to_string(dev_idx) + "/image", 100);
            stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" + std::to_string(dev_idx) + "/points", 100);
            break;
...
int CameraPublisher::onCameraOpen(AS_CAM_PTR pCamera)
{
    ...
    for (auto it = imgPubList.begin(); it != imgPubList.end(); it++)
    {
        if (it->pCamera == pCamera)
        {
            ...
            if ((cam_type == AS_SDK_CAM_MODEL_HP60C) || (cam_type == AS_SDK_CAM_MODEL_HP60CN) || (cam_type == AS_SDK_CAM_MODEL_VEGA) || (cam_type == AS_SDK_CAM_MODEL_TAISHAN) || (cam_type == AS_SDK_CAM_MODEL_TANGGULA_B))
            {
...
int CameraPublisher::setResolution(AS_CAM_PTR pCamera, LAUNCH_CONFI_PARAM_S para)
{
    ...
    if ((para.set_depth_width != -1) && (para.set_depth_height != -1) && (para.set_fps != -1))
    {
        ...
        RCLCPP_INFO_STREAM(m_logger, "set depth resolution: " << depthInfo.width << " x " << depthInfo.height << " @ " << depthInfo.fps << "fps");
    }
    ...
    if ((para.set_ir_width != -1) && (para.set_ir_height != -1) && (para.set_fps != -1))
    {
        ...
        RCLCPP_INFO_STREAM(m_logger, "set ir resolution: " << irInfo.width << " x " << irInfo.height << " @ " << irInfo.fps << "fps");
    }
    if ((para.set_peak_width != -1) && (para.set_peak_height != -1) && (para.set_fps != -1))
    {
        ...
        RCLCPP_INFO_STREAM(m_logger, "set peak resolution: " << peakInfo.width << " x " << peakInfo.height << " @ " << peakInfo.fps << "fps");
    }
    return ret;
}
...
void CameraPublisher::saveImage(const std::string &serialno, const AS_SDK_Data_s *pstData)
{
    ...
    if (pstData->depthImg.size > 0)
    {
        std::string depthImgName(std::string("depth_" + std::to_string(pstData->depthImg.width) + "x" +
                                             std::to_string(pstData->depthImg.height) + "_" + std::to_string(depthindex++) + ".yuv"));
    ...
    if (pstData->rgbImg.size > 0)
    {
        std::string rgbName(std::string("rgb_" + std::to_string(pstData->rgbImg.width) + "x" +
                                        std::to_string(pstData->rgbImg.height) + "_" + std::to_string(rgbindex++) + ".yuv"));
    ...
    if (pstData->irImg.size > 0)
    {
        std::string irName(std::string("ir_" + std::to_string(pstData->irImg.width) + "x" +
                                       std::to_string(pstData->irImg.height) + "_" + std::to_string(irindex++) + ".yuv"));
    ...
    if (pstData->peakImg.size > 0)
    {
        std::string peakName(std::string("peak_" + std::to_string(pstData->peakImg.width) + "x" +
                                         std::to_string(pstData->peakImg.height) + "_" + std::to_string(peakindex++) + ".yuv"));
                           ...
                           unsigned int idx)
{
    if (publisher)
    {
        if (publisher->get_subscription_count())
        {
            ...
            else if ((topic_name.find("rgb" + std::to_string(idx)) != std::string::npos) || (topic_name.find("mono8" + std::to_string(idx)) != std::string::npos))
            {
