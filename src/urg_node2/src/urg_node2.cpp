...
UrgNode2::CallbackReturn UrgNode2::on_configure(const rclcpp_lifecycle::State& state)
{
    ...
    error_pub_ = this->create_publisher<std_msgs::msg::Int16>("error_code", 1);
...
void UrgNode2::initialize()
{
    ...
    // メッセージヘッダのframe_id設定
    header_frame_id_ = (frame_id_.find_first_not_of('/') == std::string::npos) ? "" : frame_id_.substr(frame_id_.find_first_not_of('/'));
...
void UrgNode2::set_scan_parameter()
{
    ...
    int max_step;
    urg_step_min_max(&urg_, &min_step, &max_step);
    topic_time_increment_ = cluster_ * ((urg_step2rad(&urg_, max_step) - urg_step2rad(&urg_, min_step)) / (2.0 * M_PI)) * scan_period_ / static_cast<double>(max_step - min_step);
...
bool UrgNode2::create_scan_message(sensor_msgs::msg::LaserScan& msg)
{
    ...
    msg.header.stamp = system_time_stamp + system_latency_ + user_latency_ + get_angular_time_offset();
