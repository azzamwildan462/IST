...
frame_kamera.src = "http://" + window.location.hostname + ":8080/stream?topic=/lane_detection/frame_display"

const frame_binary = document.getElementById("frame_binary");
frame_binary.src = "http://" + window.location.hostname + ":8080/stream?topic=/lane_detection/frame_binary"
...
function send_params() {
    req_yuv = new ROSLIB.ServiceRequest({
        req_type: 1,
        req_data: [
    ...
    ser_ball_yuv.callService(req_yuv, (result) => {
    });
}
...
ser_ball_yuv = new ROSLIB.Service({
    ros: ros,
